<template>
    <li class="list-group-item">
        ...
        <select style="float:right; margin: 0px 25px"><option v-for="priority in Priority" :key="priority">{{ priority }}</option></select>
        <div v-if="modifyingTasks === task.id" style="float: right">
            <input type="text" v-model="modifiedTaskDescription" placeholder="Modify Task" aria-describedby="basic-addon2">
        ...
        methods: {
            modifyTaskDescription(task){
                if(this.modifiedTaskDescription !== ""){
                    alert('You changed ' + task.description + ' to ' + this.modifiedTaskDescription);
